...
class SitemapConnectorWeThePeopleBmx(models.AbstractModel):
    ...
    def get_product_entries(self, source, category_filter=None, limit=0):
        try:
            ...
            _logger.warning(
                'WeThePeople: el sitemap no pudo procesarse; se usa el catálogo como '
                'respaldo: %s', exc,
            )
        ...
        if not entries_by_key:
            raise ValueError(
                'No se localizaron fichas de bicicletas, cuadros o componentes '
                'en el sitemap de WeThePeople.'
            )
