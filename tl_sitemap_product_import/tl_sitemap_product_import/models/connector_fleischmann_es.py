...
class SitemapConnectorFleischmannEs(models.AbstractModel):
    """Conector del catálogo español de Fleischmann bajo ``/fes/``.

    La tienda actual es Magento y publica el catálogo paginado en
    ``/fes/productos.html``. Las fichas siguen el patrón
    ``/fes/productos/<categorías>/<referencia>-<slug>.html``.
    """

    _name = 'sitemap.connector.fleischmann_es'
    _inherit = 'sitemap.import.service'
    _description = 'Conector Fleischmann España'

    _HOSTS = {'fleischmann.de', 'www.fleischmann.de'}
    _PRODUCT_RE = re.compile(
        r'^/fes/productos/(?:[^/?#]+/)*(?P<code>\d{5,8})-[^/?#]+\.html/?$',
        re.IGNORECASE,
    )
    ...
    def _canonical_url(cls, value):
        ...
        if host != 'fleischmann.de':
            return False
        path = re.sub(r'/+', '/', parts.path or '/')
        return urlunsplit(('https', 'www.fleischmann.de', path.rstrip('/'), '', ''))
    ...
    @classmethod
    def _product_links(cls, tree, page_url):
        links = []
        xpaths = (
        ...
        for xpath in xpaths:
            for href in tree.xpath(xpath):
                ...
                if cls._product_match(absolute) and absolute not in links:
    ...
    def _discover_catalog(self, source, limit=0):
        ...
        for page in range(1, 1000):
            ...
            if page > 1:
                query['p'] = str(page)
            page_url = 'https://www.fleischmann.de/fes/productos.html?' + urlencode(query)
            try:
                response = self._http_get(session, page_url, source)
                tree = lxml_html.fromstring(response.content)
            except Exception as exc:
                _logger.info('Fleischmann: página de catálogo no accesible %s: %s', page_url, exc)
                break
    ...
    def get_product_entries(self, source, category_filter=None, limit=0):
        ...
        if not result:
            raise ValueError(
                'No se encontraron fichas Fleischmann bajo /fes/productos.html. '
    ...
    def _attributes(cls, tree, name):
        ...
        if scale:
            scale_code = scale.group(1).upper().replace('H0E', 'H0e')
            numeric_scale = {
                'N': '1:160',
                'H0': '1:87',
                'H0e': '1:87',
            ...
            if numeric_scale:
                attrs.setdefault('Escala', [numeric_scale])
        for token, label in (
            ...
            if token.casefold() in focused.casefold():
                attrs.setdefault(label, [])
                value = token.upper() if token in {'DCC', 'Z21', 'DC'} else 'Sí'
                if value not in attrs[label]:
                    attrs[label].append(value)
        return attrs
    ...
    def fetch_preview(self, source, url):
        ...
        if not match:
            raise ValueError('La URL de Fleischmann ya no corresponde a una ficha de producto bajo /fes/.')
        ...
        if not breadcrumbs:
            ...
            breadcrumbs = ['Fleischmann'] + [part.replace('-', ' ').title() for part in parts]
        ean_variants = []
