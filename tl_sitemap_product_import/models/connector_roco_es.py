...
class SitemapConnectorRocoEs(models.AbstractModel):
    ...
    _PRODUCT_RE = re.compile(
        r'^/res/productos/(?:[^/?#]+/)*(?P<code>\d{5,8})-[^/?#]+\.html/?$',
        re.IGNORECASE,
    )
    ...
    def fetch_preview(self, source, url):
        ...
        if not match:
            raise ValueError('La URL de ROCO ya no corresponde a una ficha de producto bajo /res/.')
