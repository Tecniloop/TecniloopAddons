...
class SitemapConnectorJouefUk(models.AbstractModel):
    """Conector para el catálogo británico de Jouef.

    Jouef usa la plataforma de Hornby Hobbies. ``robots.txt`` declara
    ``https://uk.jouef.com/sitemap.xml`` y las fichas usan URLs planas::

        /products/jouef-h0-187-sncf-electric-locomotive-class-bb-12000-green-and-yellow-livery-bb-12079-period-iv-hj2338
        /products/jouef-h0-187-thalys-pbka-4-unit-pack-period-vi-hj2358
    ...
    de "Product Info" a ``description_ecommerce`` y convierte la
    tabla técnica en atributos ``no_variant`` mediante el flujo común.
    """

    _name = 'sitemap.connector.jouef_uk'
    _inherit = 'sitemap.import.service'
    _description = 'Conector Jouef UK'

    _HOST = 'uk.jouef.com'
    _EUR_HOST = 'fr.jouef.com'
    ...
    _PRICE_RE = re.compile(
        ...
        r'(?P<suffix>\d[\d.,\s]*\d|\d)\s*(?:GBP|£))',
        re.IGNORECASE,
    )
    _EUR_PRICE_RE = re.compile(
        r'(?:€\s*(?P<prefix>\d[\d.,\s]*\d|\d)|'
        r'(?P<suffix>\d[\d.,\s]*\d|\d)\s*(?:EUR|€))',
        re.IGNORECASE,
    )
    ...
    _SPEC_NAME_MAP = {
        ...
        'livery/owner logo': 'Compañía ferroviaria',
        ...
        'operator': 'Compañía ferroviaria',
    }
    _SPEC_STOPPERS = {
        'avisos de seguridad', 'safety notices', 'safety information', 'comentarios',
        'reviews', 'customer support', 'quantity', 'cantidad',
    ...
    _DESCRIPTION_STOPPERS = {
        ...
        'tech specs', 'technical specifications', 'reviews',
        'comentarios', 'quantity', 'cantidad',
    }
    ...
    @classmethod
    def _code_from_product_url(cls, value):
        raw = html.unescape(str(value or '')).strip().replace('\\/', '/')
        if not raw:
            return False
        match = cls._PRODUCT_RE.match(urlparse(raw).path)
        if not match:
            return False
    ...
    def _product_key(cls, value):
        ...
        if not match:
            return False
        return cls._code_from_product_url(value)

    @classmethod
    def _eur_equivalent_url(cls, value):
        canonical = cls._canonical_url(value)
        if not canonical:
            return False
        parts = urlsplit(canonical)
        if not cls._PRODUCT_RE.match(parts.path):
            return False
        return urlunsplit(('https', cls._EUR_HOST, parts.path, '', ''))
    ...
    def _candidate_sitemaps(self, source):
        ...
        for candidate in (
            ...
            try:
                ...
                _logger.info('Jouef: no se pudo consultar %s: %s', candidate, exc)
        candidates.extend([
            'https://uk.jouef.com/sitemap.xml',
            'https://uk.jouef.com/sitemap_index.xml',
        ])
        return list(dict.fromkeys(candidates))

    def _iter_sitemap_entries(self, source, sitemap_url, depth=0, visited=None):
        if depth > 10:
            raise ValueError('El sitemap de Jouef supera diez niveles de índices.')
    ...
    def _fallback_catalog_entries(self, source, limit=0):
        ...
        start_urls = [
            'https://uk.jouef.com/catalogue',
            'https://uk.jouef.com/catalogue/trains-train-sets',
            'https://uk.jouef.com/catalogue/trains-train-sets/locomotives',
            'https://uk.jouef.com/catalogue/trains-train-sets/coaches-coach-packs',
            'https://uk.jouef.com/catalogue/trains-train-sets/wagons-wagon-packs',
            'https://uk.jouef.com/catalogue/track-and-power',
            'https://uk.jouef.com/catalogue/buildings-accessories',
            'https://uk.jouef.com/catalogue/epoch',
        ]
        ...
        while queue and len(visited) < 350:
            ...
            try:
                ...
                _logger.info('Jouef: catálogo no accesible %s: %s', page_url, exc)
                continue
    ...
    def _collect_products(self, source):
        ...
        for sitemap_url in self._candidate_sitemaps(source):
            try:
                ...
                _logger.info('Jouef: sitemap no utilizable %s: %s', sitemap_url, exc)
    ...
    def _clean_breadcrumbs(cls, values, product_name):
        excluded = {'inicio', 'home', 'jouef', 'productos', 'products', 'shop', 'tienda'}
    ...
    _CATEGORY_TRANSLATIONS = {
        'trains & train sets': 'Trenes y sets',
        'trains and train sets': 'Trenes y sets',
        'train sets': 'Sets de trenes',
        'locomotives': 'Locomotoras',
        'electric': 'Eléctricas',
        'diesel': 'Diésel',
        'steam': 'Vapor',
        'coaches and coach packs': 'Coches y sets de coches',
        'passenger coaches': 'Coches de viajeros',
        'wagons and wagon packs': 'Vagones y sets de vagones',
        'track & power': 'Vías y alimentación',
        'track and power': 'Vías y alimentación',
        'track pieces': 'Vías',
        'digital command control': 'Control digital DCC',
        'buildings & accessories': 'Edificios y accesorios',
        'buildings and accessories': 'Edificios y accesorios',
        'publications & catalogues': 'Publicaciones y catálogos',
        'publications and catalogues': 'Publicaciones y catálogos',
        'spares': 'Repuestos',
    ...
    def _translate_categories(cls, values):
        ...
        for raw in values or []:
            value = cls._normalise_text(raw)
            mapped = cls._CATEGORY_TRANSLATIONS.get(value.casefold(), value)
    ...
    def _parse_amount(cls, value):
        text = cls._normalise_text(value).replace('£', '').replace('GBP', '').replace('\xa0', ' ').strip()
        if not text:
            return 0.0
    ...
    @classmethod
    def _price_from_page(
        cls, tree, lines, product_node, product_name,
        default_currency='GBP', price_pattern=None,
    ):
        prices = []
        currency = default_currency
        price_pattern = price_pattern or cls._PRICE_RE
    ...
    def _official_eur_price(self, source, session, uk_product_url, expected_code):
        """Obtiene el precio oficial EUR mediante la capa común de Hornby."""
        return self._hornby_official_eur_price(
            source=source,
            session=session,
            product_url=uk_product_url,
            expected_code=expected_code,
            eur_hosts=(self._EUR_HOST,),
    ...
    def _spec_attributes(cls, lines):
        ...
        spec_lines = cls._section_lines(
            lines,
            ('Especificaciones técnicas', 'Technical specifications', 'Tech Specs'),
            cls._SPEC_STOPPERS,
        )
        if not spec_lines:
            return attributes
    ...
    def _inferred_category(cls, name, product_type=False):
        ...
        if any(token in text for token in ('locomotora', 'locomotive')):
            ...
            if any(token in text for token in ('eléctrica', 'electrica', 'electric')):
                ...
                result.append('Vapor')
            return result
        if any(token in text for token in ('automotor', 'unidad ', 'electrotrén', 'electrotren basculante', 'railcar', 'emu', 'multiple unit')):
            return ['Trenes y Set', 'Automotores y unidades']
        if any(token in text for token in ('coche de viajeros', 'coche ', 'coach', 'coaches')):
            return ['Trenes y Set', 'Coches y set']
        if any(token in text for token in ('vagón', 'vagon', 'wagon', 'wagons', 'freight car')):
    ...
    def fetch_preview(self, source, url):
        ...
        if not self._product_match(canonical_response):
            raise ValueError(
                'Jouef redirigió la ficha a una página que no es un producto: '
                f'{response.url}'
            )
        ...
        if not name:
            raise ValueError('La ficha de Jouef no contiene un nombre de producto.')

        style_code = self._item_code(tree, lines, product_node, canonical_response)
        # El catálogo se descubre en la tienda UK, pero el precio se toma de
        # la ficha oficial francesa para importar el importe comercial real en EUR.
        # No se aplica ninguna conversión de divisa.
        price, currency, price_available = self._official_eur_price(
            source, session, canonical_response, style_code,
        )
        ...
        contains_lines = self._section_lines(
            lines,
            ('Qué contiene', 'Que contiene', "What's inside", 'Contents'),
            {'recomendado para ti', 'recommended for you', 'especificaciones técnicas',
             'tech specs', 'reviews', 'comentarios', 'quantity', 'cantidad'},
        )
        ...
        if breadcrumbs:
            category_segments = self._translate_categories(breadcrumbs)
        else:
