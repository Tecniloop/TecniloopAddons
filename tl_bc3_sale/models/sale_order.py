...
class SaleOrder(models.Model):
    ...
    def _prepare_sale_line_from_bc3_line(self, budget_line, product):
        ...
        if budget_line.line_type in ("resource", "percentage"):
            # Keep decomposed resources in the technical budget, not in the commercial quotation.
            return {
                "display_type": "line_note",
                "name": _("BC3 decomposition: %(code)s - %(name)s") % {"code": budget_line.code, "name": budget_line.name},
                "sequence": budget_line.sequence,
                "bc3_budget_line_id": budget_line.id,
                "bc3_code": budget_line.code,
                "bc3_position_path": budget_line.position_path,
