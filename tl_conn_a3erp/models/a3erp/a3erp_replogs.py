...
class A3erpRepLogs(models.Model):
    ...
    def getall_records(self, tabla, url, modelo, campoClave, campoClave2 = False, largeImport = False, **kwargs):
        ...
        for company in self.env.company:
            ...
            try:                       
                while True:
                    if kwargs.get('fecha_alta'): # NOTE: Articulos según fecha alta
                        cola_state = MensajeSolicitudGet(company.a3erp_company_id, campos, str(kwargs.get('fecha_alta').date().strftime("%d/%m/%Y")))    
                    elif kwargs.get('fecha_mod') and kwargs.get('campo'): # NOTE: Articulos según fecha modificacion y CARAC
                        clavesextra = [str(kwargs.get('campo')), str(kwargs.get('fecha_mod').date().strftime("%d/%m/%Y"))]
                        cola_state = MensajeSolicitudGet(company.a3erp_company_id, campos, "ARTICULO", clavesextra)                            
                    else: # NOTE: Comportamiento normal
