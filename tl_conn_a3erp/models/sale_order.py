...
GET_CLIENTENIF = "{}/clientes/getbynifcli"
...
class SaleOrder(models.Model):
    ...
    def action_a3erp_send_confirm(self):
        """Mostrar wizard para confirmar el envío, aprovechar y controlar si el cliente existe en a3ERP mediante NIF."""
        self.ensure_one()
        headers = {
        ...
        if not self.partner_id.cod_cliente_a3 and self.partner_id.vat:
            ...
            response_cliente = requests.post(
                GET_CLIENTENIF.format(self.company_id.a3erp_url),
            ...
            if response_cliente.status_code != 200:
                ...
                return {
                    ...
                    'params': {
                        'title': _('Error'),
                        'type': 'danger',
                        'message': f"Error {response_cliente.status_code}: {response_cliente.text}"
                        #'sticky': True,
                    }
                }
            ...
            if cliente_data.get('result'):
                cod_cliente_a3 = cliente_data.get('result')[0].get('CODCLI').strip()
                self.update_partner_codcli(cod_cliente_a3)
                
                partner_ids = self.partner_id.search([('cod_cliente_a3', '=', cod_cliente_a3)])
                dst_partner = max(partner_ids, key=lambda r: r.id)
                del_partner = min(partner_ids, key=lambda r: r.id).name
                
                if partner_ids and len(partner_ids) > 1:
                    try:
                        self.env['base.partner.merge.automatic.wizard']._merge(partner_ids.ids, dst_partner)
                        self.message_post(body=f"Cliente '{dst_partner.name}' fusionado con el Cliente potencial '{del_partner}'.")
                        _logger.info(f"Cliente '{dst_partner.name}' fusionado con el Cliente potencial '{del_partner}'.")
                    
                    except Exception as e:
                        _logger.info(f"Error Fusionado Clientes: {format(e)}")
                        raise UserError(_("Error al fusionar clientes: %s") % format(e))
                else:
                    _logger.info(f"Cliente {self.partner_id.name} actualizado con el CODCLI de a3ERP {cod_cliente_a3}.")
                    # self.partner_id.get_a3erp_contact_data()
            else:
                _logger.info(f"Cliente {self.partner_id.name}, no se ha encontrado un cliente con NIF B65359382")
            ...
            try:
                    ...
                    if line.display_type in ('line_section', 'line_subsection'): # SECCION
                        ...
                        if not line.product_id.cod_articulo_a3:
                            ...
                            for field in required_lineofer:
                                ...
                                elif 'discount' in field.odoo_field_name:
                                    campo_a3 = MAPPER.traducir_campo(line._fields[field.odoo_field_name].type)
                                    linea.append(Parametro(field.a3erp_field_name, line[field.odoo_field_name], campo_a3))
                ...
                if empty_codart or break_record:
                    if break_record:
                        ...
                        self._log_and_continue(record, msg)
                        raise ValidationError(_(msg))
                    raise ValidationError(_("Hay un Articulo que no tiene un CODART."))
    ...
    def _onchange_partner_id_show_update_price_buton(self):
        ...
        try:
            ...
            riesgo_data = partner.get_a3erp_riesgo_cliente()
            if not riesgo_data:
                return
    ...
    def action_refresh_partner_credit(self):
        if not self.partner_id:
            self.a3erp_credit = 0.0
            self.a3erp_max_credit= 0.0
            return
        ...
        try:
            ...
            raise UserError(_(f"Error recuperando el Riesgo: {format(e)} "))
