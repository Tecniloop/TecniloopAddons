...
class TestL10nEsAeatVatBookOss(TestL10nEsAeatVatBook):
    ...
    def setUpClass(cls):
        ...
        cls.general_tax = cls.company._get_taxes_from_xmlids(
            ["account_tax_template_s_iva21b"]
        )[:1]
        if not cls.general_tax:
            cls.general_tax = cls.env["account.tax"].search(
                [
                    ("company_id", "=", cls.company.id),
                    ("type_tax_use", "=", "sale"),
                    ("amount", "=", 21),
        ...
        cls.accounts["700000"].write({"tax_ids": [Command.link(cls.general_tax.id)]})

    def test_model_vat_book_oss(self):
        sp_fiscal_position = self.env.ref(
            f"account.{self.company.id}_fp_nacional", raise_if_not_found=False
        )
        if not sp_fiscal_position:
            sp_fiscal_position = self.env["account.fiscal.position"].search(
                [
        ...
        if sp_fiscal_position:
            data["fiscal_position_id"] = sp_fiscal_position.id
