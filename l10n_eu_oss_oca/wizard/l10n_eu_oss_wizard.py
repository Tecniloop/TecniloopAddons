...
class L10nEuOssWizard(models.TransientModel):
    ...
    def _prepare_fiscal_position_vals(self, country, tax_ids):
    ...
    def update_fpos(self, fpos_id, tax_ids):
        oss_tax_ids = fpos_id.tax_ids.filtered("oss_country_id")
        kept_tax_ids = (fpos_id.tax_ids - oss_tax_ids).ids
        fpos_id.write({"tax_ids": [Command.set(kept_tax_ids + tax_ids)]})

    def _update_tax_mapping(self, tax_mapping):
        account_tax = self.env["account.tax"]
        for tax_dest_id, tax_src_ids in tax_mapping.items():
            account_tax.browse(tax_dest_id).write(
                {"original_tax_ids": [Command.set(list(dict.fromkeys(tax_src_ids)))]}
            )
        ...
        for country in self.todo_country_ids:
            ...
            for tax, rate in dict_taxes.items():
                if last_rate != rate:
                    ...
                    if not tax_dest_id:
                        tax_group_vals = self._prepare_tax_group_vals(rate)
                        tax_group = account_tax_group.search(
                            [
                                ("name", "=", tax_group_vals["name"]),
                                ("country_id", "=", tax_group_vals["country_id"]),
                                ("company_id", "=", tax_group_vals["company_id"]),
                            ],
                            limit=1,
                        )
                        if not tax_group:
                            tax_group = account_tax_group.create(tax_group_vals)
                        tax_dest_id = account_tax.create(
                            self._prepare_tax_vals(country, tax, rate, tax_group)
                        )
                tax_ids.append(tax_dest_id.id)
                tax_mapping.setdefault(tax_dest_id.id, []).append(tax.id)
                last_rate = rate
            ...
            self._update_tax_mapping(tax_mapping)
            if not fpos:
                data_fiscal = self._prepare_fiscal_position_vals(country, tax_ids)
                fpos_obj.create(data_fiscal)
            else:
                self.update_fpos(fpos, tax_ids)
